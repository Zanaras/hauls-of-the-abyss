...
/**
 * User Controller is for anything that creates, modifies, or removes user accounts or user controls, lets someone login or logout, or lets someone access something that only makes sense if you're logged in but not playing.
 */
class UserController extends AbstractController {
	...
	public function __construct(AppState $app, EntityManagerInterface $em, EmailVerifier $emailVerifier, TranslatorInterface $trans) {
		$this->app = $app;
		$this->em = $em;
	...
	public function reset(AppState $app, MailManager $mail, TranslatorInterface $trans, Request $request, UserPasswordHasherInterface $passwordHasher, string $token = '0', string $email = '0'): RedirectResponse|Response {
		if ($token == '0') {
			$form = $this->createForm(RequestResetFormType::class);
			$form->handleRequest($request);
			if ($form->isSubmitted() && $form->isValid()) {
				$data = $form->getData();
				$user = $this->em->getRepository(User::class)->findOneByEmail($data['text']);
				if (!$user) {
					$user = $this->em->getRepository(User::class)->findOneByUsername($data['text']);
				}
				if ($user) {
			...
			$user = $this->em->getRepository(User::class)->findOneBy(['reset_token' => $token, 'email' => $email]);
			if ($user) {
				...
				$app->logUser($user, 'user_reset',[], true, 'sl');
				return new RedirectResponse($this->generateUrl('public_index'));
			}
		}
	}
	...
	#[Route(path: '/user/characters', name: 'user_characters')]
